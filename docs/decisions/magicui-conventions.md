...
| App shell (`AppLayout.tsx`) | None (deliberate) | Rendered on every authenticated page, so any accent here would be visible constantly, violating "restrained" by definition, and a `BorderBeam` here would collide with the one already placed on each page's own headline element (Analytics' stat tile, Login/Signup's card) — a nav bar is also a poor fit for a once-per-mount `BlurFade` since it doesn't want to visibly re-enter on every client-side route change the way page content does. Left untouched. |
